...
(function () {
  ...
  function setMetaTags() {
    ...
    setMeta('property', 'og:image', 'images/og/v3.jpg');
  ...
  function initFallingLeaves() {
    ...
    const leafColors = [
      { fill: 'rgba(82, 138, 174, 0.6)', stroke: 'rgba(46, 89, 132, 0.3)' },    // 메인 블루
      { fill: 'rgba(46, 89, 132, 0.5)', stroke: 'rgba(30, 63, 102, 0.3)' },     // 네이비
      { fill: 'rgba(115, 165, 198, 0.5)', stroke: 'rgba(82, 138, 174, 0.3)' },  // 밝은 블루
      { fill: 'rgba(188, 210, 232, 0.5)', stroke: 'rgba(115, 165, 198, 0.3)' }, // 가장 밝은 하늘색
      { fill: 'rgba(145, 186, 214, 0.5)', stroke: 'rgba(46, 89, 132, 0.3)' },   // 중간 하늘색
    ];
  ...
  /* ═══════════════════════════════════════════
     Contact Section (연락하기)
     ═══════════════════════════════════════════ */

  function initContact() {
    // 전화번호를 받아서 HTML의 <a> 태그 href에 tel: 과 sms: 를 붙여주는 핵심 함수입니다.
    const bindContact = (prefix, name, phone) => {
      const nameEl = $(`#${prefix}-name-pop`);
      const callEl = $(`#${prefix}-call-pop`);
      const smsEl = $(`#${prefix}-sms-pop`);

      if (nameEl && name) nameEl.textContent = name;
      
      // 모바일 기기에서 이 버튼을 누르면 전화/문자 앱이 열리게 됩니다.
      if (callEl && phone) callEl.href = `tel:${phone}`;
      if (smsEl && phone) smsEl.href = `sms:${phone}`;
    };
    ...
    // 신랑측 연락처 연결
    bindContact('groom', g.name, g.phone);
    bindContact('groom-father', g.father, g.fatherPhone);
    bindContact('groom-mother', g.mother, g.motherPhone);

    // 신부측 연락처 연결
    bindContact('bride', b.name, b.phone);
    bindContact('bride-father', b.father, b.fatherPhone);
    bindContact('bride-mother', b.mother, b.motherPhone);

    // 계좌번호에서 사용했던 아코디언 애니메이션을 연락하기 버튼에도 적용합니다.
    initAccordion('contactGroomAccordion', 'contactGroomAccordionPanel');
    initAccordion('contactBrideAccordion', 'contactBrideAccordionPanel');
  }
...
/* ═══════════════════════════════════════════
     Photo Modal (Swipe, Pinch Zoom, Double Tap)
   ═══════════════════════════════════════════ */
  ...
  function updateImageTransform() {
    ...
    // 확대 상태가 아닐 때는 부드러운 전환 효과(transition) 적용, 확대 중일 때는 즉각 반응하도록 제거
    ...
    img.style.transform = `translate3d(${currentX}px, ${currentY}px, 0) scale(${currentScale})`;
  }

  function showModalImage() {
    const img = $('#modalImg');
    resetZoomState(); // 사진이 바뀔 때마다 확대 상태 초기화
    img.src = modalImages[modalIndex];
    $('#modalCounter').textContent = `${modalIndex + 1} / ${modalImages.length}`;
  ...
  // 두 터치 지점 간의 거리를 계산하는 헬퍼 함수 (핀치 투 줌용)
  ...
  function initPhotoModal() {
    ...
    const modal = $('#photoModal');
    // 모달 바깥 영역 클릭 시 닫기 (이미지를 클릭했을 때는 안 닫히도록)
    modal.addEventListener('click', (e) => {
      if (e.target === modal || e.target.id === 'modalContainer') {
        closePhotoModal();
      }
    ...
    // PC 마우스 휠을 통한 확대/축소 (선택 사항)
    container.addEventListener('wheel', (e) => {
        e.preventDefault();
        const delta = e.deltaY > 0 ? -0.1 : 0.1;
        currentScale = Math.min(Math.max(1, currentScale + delta), 4); // 최대 4배
        updateImageTransform();
    }, { passive: false });
  }
  ...
  function handleTouchStart(e) {
    if (e.touches.length === 2) {
        ...
        if (tapLength < 300 && tapLength > 0) {
            // 더블 탭: 확대된 상태면 축소, 축소된 상태면 2.5배 확대
            e.preventDefault();
            if (currentScale > 1) {
                resetZoomState();
            } else {
            ...
            if (currentScale > 1) {
                // 확대된 상태에서는 이미지 이동(Panning)
                isPanning = true;
                startX = touchStartX - currentX;
                startY = touchStartY - currentY;
            }
        }
  ...
  function handleTouchMove(e) {
    if (isZooming && e.touches.length === 2) {
        ...
        // 확대 배율 계산 (최대 4배 제한)
        currentScale = Math.min(Math.max(1, startScale * (currentDistance / startDistance)), 4);
        updateImageTransform();
    } else if (isPanning && currentScale > 1 && e.touches.length === 1) {
        // 확대된 상태에서 이미지 이동 중
        e.preventDefault();
        const touch = e.touches[0];
        
        // 이동 범위 계산
        currentX = touch.screenX - startX;
        currentY = touch.screenY - startY;
  ...
  function handleTouchEnd(e) {
    ...
    } else if (e.changedTouches.length === 1 && currentScale === 1) {
        // 확대되지 않은 상태에서 단일 터치 종료 시 스와이프 동작 처리
        touchEndX = e.changedTouches[0].screenX;
        touchEndY = e.changedTouches[0].screenY;
        handleSwipe();
    }
  ...
  function handleSwipe() {
    // 확대된 상태에서는 스와이프로 사진이 넘어가지 않도록 차단
      ...
      btnKakaoShare.addEventListener('click', () => {
        Kakao.Share.sendDefault({
          ...
          content: {
            ...
            imageUrl: 'https://hycho039.github.io/wedding_260830/images/og/v3.jpg',
            link: {
